...
CREATE OR REPLACE FUNCTION public.create_sale(p_bill_number text, p_subtotal numeric, p_discount numeric, p_total numeric, p_payment_method text, p_customer_id uuid DEFAULT NULL::uuid, p_items jsonb DEFAULT '[]'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Insert sale items (trigger reduce_stock_on_sale handles stock deduction)
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
  LOOP
